...
## When to Use This Skill vs `requesting-code-review`

| | `review` (this skill) | `requesting-code-review` |
|---|---|---|
| **When** | At completion, before final merge | During active development, after each subtask |
| **Scope** | Comprehensive: Standards + Spec dual-axis | Quick checkpoint: catches issues before they cascade |
| **Input** | Fixed-point diff + spec doc + standards docs | Git SHAs + brief task description |
| **Output** | Two independent reports (Standards + Spec) | Actionable issues (Critical/Important/Minor) |
| **Speed** | Thorough, may take longer | Fast, focused on the latest change |
| **Goal** | Holistic pass/fail against requirements and conventions | Prevent compounding issues; keep momentum |

**Use this skill** when work is complete and you need a final sign-off against both the spec and the repo's coding standards. This is the "before merge" gate.

**Use `requesting-code-review`** during development for quick sanity checks after each subtask. It's lighter-weight and designed to keep you moving.

They complement, not compete: use `requesting-code-review` throughout development; use `review` as the final gate.
